...
// limitations under the License.
  ...
  extension Clients {
    ...
    class ExamplesTransport: ExamplesStub {
      ...
      public func createExample(
        ...
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDialogflowCxV3.Example.self, from: data)
      }
      ...
      public func listExamples(
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDialogflowCxV3.ListExamplesResponse.self, from: data)
      }
        ...
        let path = try { () throws -> Swift.String in
          guard let pathVariable0 = request.example.map({ $0.name }), !pathVariable0.isEmpty else {
            throw GoogleCloudGax.RequestError.binding(
              "'request.example.name' is not set or is empty")
          }
          return "/v3/\(pathVariable0)"
        }()
        var query = [
