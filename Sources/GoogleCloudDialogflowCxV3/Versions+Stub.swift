...
// limitations under the License.
  ...
  extension Clients {
    ...
    class VersionsTransport: VersionsStub {
      ...
      public func listVersions(
        ...
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDialogflowCxV3.ListVersionsResponse.self, from: data)
      }
      ...
      public func getVersion(
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDialogflowCxV3.Version.self, from: data)
      }
        ...
        let path = try { () throws -> Swift.String in
          guard let pathVariable0 = request.version.map({ $0.name }), !pathVariable0.isEmpty else {
            throw GoogleCloudGax.RequestError.binding(
              "'request.version.name' is not set or is empty")
          }
          return "/v3/\(pathVariable0)"
        }()
        var query = [
      ...
      public func compareVersions(
        ...
        let path = try { () throws -> Swift.String in
          guard let pathVariable0 = request.baseVersion as Swift.String?, !pathVariable0.isEmpty
          else {
            throw GoogleCloudGax.RequestError.binding(
              "'request.base_version' is not set or is empty")
          }
          return "/v3/\(pathVariable0):compareVersions"
        }()
        let query = [
          URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
        ]
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDialogflowCxV3.CompareVersionsResponse.self, from: data)
      }
