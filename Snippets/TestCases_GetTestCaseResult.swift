...
// limitations under the License.
  ...
  func sample(
    ...
    testCaseId: String, resultId: String
  ) async throws {
    let response = try await client.getTestCaseResult(
      request: GetTestCaseResultRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/agents/\(agentId)/testCases/\(testCaseId)/results/\(resultId)"
        }
    )
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          agentId: "[placeholder]", testCaseId: "[placeholder]", resultId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }
